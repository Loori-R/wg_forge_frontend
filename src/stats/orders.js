const result_html = (obj, currency) => {
    if (!currency) { currency = '$' }
    const result_td = {}
    _.map(obj, (item, index) => {
        if (item <= 0 || isNaN(item)) { item = 'n/a' }
        else if (index !== 'orders_count') { item = `${currency} ${item}` }
        result_td[index] = `<td colspan=6>${item}</td>`
    })
    return result_td
}

const order_stats = (getter, calc_median, currency) => {
    ...
    return result_html(result, currency)
}
